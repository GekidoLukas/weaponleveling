...
public class CustomToast implements Toast {
    ...
    public Visibility render(GuiGraphics guiGraphics, ToastComponent component, long visibilityTime) {
        ...
        guiGraphics.blit(CUSTOM_TEXTURE, 0, ypos, 0, 0, this.width(), this.height());
        ...
        if (stack != null) {
            ...
            if (subtitle.size() == 1) {
                guiGraphics.drawString(component.getMinecraft().font, title.get(0), 35, ypos+7, i | -16777216);
                guiGraphics.drawString(component.getMinecraft().font, title.get(0), 35, ypos+7, i | -16777216);
                guiGraphics.drawString(component.getMinecraft().font, subtitle.get(0), 35, ypos+16, -1);
                guiGraphics.drawString(component.getMinecraft().font, subtitle.get(0),35, ypos+16, -1);
            } else {
