package net.weaponleveling.fabric.config;
...
public class WeaponLevelingConfigFabric {
    ...
    public static class Server {
        ...
        static {
            ...
            value_bowlike_damage_modifier= BUILDER.comment("The Value, that gets multiplied with the bow's damage to lower it. It is recommended to leave it below 1  (Default = 0.1)").defineInRange("weaponBowLikeModifier",0.25d, 0, 10);
            ...
            blacklist_items = BUILDER.comment("Blacklisted Items").defineList("blacklist_items", ConfigListsFabric.DEFAULT_ITEM_BLACKLIST, o -> o instanceof String);
            melee_items = BUILDER.comment("Melee Weapons, that are not extending the AxeItem or SwordItem Class").defineList("melee_items", ConfigListsFabric.DEFAULT_MELEE_ITEMS, o -> o instanceof String);
            armor_items = BUILDER.comment("Armors, that are not extending the Armor Class or Should be added if Option \"disable_unlisted_items\" is set to true").defineList("armor_items", ConfigListsFabric.DEFAULT_ARMOR_ITEMS, o -> o instanceof String);
            projectile_items = BUILDER.comment("Projectile Weapons, that are not extending the AxeItem or SwordItem Class").defineList("projectile_items", ConfigListsFabric.DEFAULT_PROJECTILE_ITEMS, o -> o instanceof String);
            ...
            entities_animal = BUILDER.comment("A List of all the Animals affected by the mod").defineList("animal_entities", ConfigListsFabric.DEFAULT_ANIMALS, o -> o instanceof String);
            entities_monster = BUILDER.comment("A List of all the Monsters affected by the mod").defineList("monster_entities", ConfigListsFabric.DEFAULT_MONSTER, o -> o instanceof String);
            entities_miniboss = BUILDER.comment("A List of all the Mini Bosses affected by the mod").defineList("miniboss_entities", ConfigListsFabric.DEFAULT_MINIBOSSES, o -> o instanceof String);
            entities_boss = BUILDER.comment("A List of all the Bosses affected by the mod").defineList("boss_entities", ConfigListsFabric.DEFAULT_BOSSES, o -> o instanceof String);
